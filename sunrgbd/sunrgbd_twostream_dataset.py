""" Labled and unlabeled dataset for Self-Ensembling Semi-Supervised 3D Object Detection

Author: Zhao Na, Oct 2019
"""
import os
import sys
import random
...
class SunrgbdUnlabedledTwoStreamDataset(Dataset):
    def __init__(self, labeled_sample_list=None, num_points=20000, use_color=False, use_height=False, use_v1=False,
                 augment=False, scan_idx_list=None):
        print('----------------Sunrgbd Unlabedled TwoStream Dataset Initialization----------------')
        if use_v1:
            self.data_path = os.path.join(ROOT_DIR, 'sunrgbd/sunrgbd_pc_bbox_votes_50k_v1_train')
        else:
            self.data_path = os.path.join(ROOT_DIR, 'sunrgbd/sunrgbd_pc_bbox_votes_50k_v2_train')
